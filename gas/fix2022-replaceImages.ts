...
function replaceImages() {
  const body = DocumentApp.openByUrl(
    'https://docs.google.com/document/d/1VTRe6xdf9Jhru01QGWhcVvyWSMVlIqSTgA2Gukh2ZSw/edit'
  ).getBody()

  const map: Record<
    string,
    { imageContainer: GoogleAppsScript.Document.Paragraph; currentInline: GoogleAppsScript.Document.InlineImage } | null
  > = Object.fromEntries(imagesToReplace.map(imageName => [imageName.replace(/^figure-|\.png$/g, ''), null]))
  ...
  for (let i = 0; i < len; i++) {
    ...
    if (map[figNum] === undefined) continue
  ...
  for (const [figNum, obj] of Object.entries(map)) {
    if (!obj) throw new Error(`figure ${figNum} not found in document`)
  }

  Logger.log('all images found, start replacing...')

  for (const [figNum, obj] of Object.entries(map)) {
    if (!obj) throw new Error('assertion error')
